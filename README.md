...
#### ========== Встреча 18.01 ============
### Ещё раз про постановку задачи
- динамическая задача

<img src="Images/active_hosts_18_01.png" width="700" height="300">

- пустой кластер

- приходят виртуальные машины VM(CPU, RAM, ..., time_end)

- online: отправляем VM на физический сервер

- (!) дорогой тариф (не перемещаем VM между серверами)

- (!) заранее знаем время работы VM

- baseline: online dynamic vector packing

- оптимизирую потребление энергии по сравнению с best-fit

### Про параметры симуляции

- VM(CPU, RAM, ..., time_begin, time_end)

- Промежуток между приходами VM: Poisson(a)

- Начинаем с пустым кластером, но оптимизируем промежуток

<img src="Images/cpu_util_18_01.png" width="700" height="300">

- Всего есть 5 сценариев

- 2 на изменение интенсивности

- 3 на изменение типов VM
